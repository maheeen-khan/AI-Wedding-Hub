...
/** Route: /vendor/makeup/:id */
export default function MakeupPage() {
  const { id } = useParams();
  const vendor = id && id !== makeupData.id ? null : makeupData;

  if (!vendor) return <Navigate to="/" replace />;

  const handleBookNow = (formData) => {
    console.log("Makeup booking submitted:", { vendorId: vendor.id, ...formData });
  };

  return (
    <div className="vp-page">
      <Navbar breadcrumbLabel={vendor.breadcrumbLabel} breadcrumbHref="/vendors/makeup" />
      ...
      <main className="vp-page__main">
        ...
        <div className="vp-page__full-width">
          <MenuPackagesTable title="Signature Packages" packages={vendor.signaturePackages} />
          <EnhancementsList items={vendor.enhancements} />
          <ImportantNotice text={vendor.importantNotice} />
          <ReviewsSection title="What Brides Say" reviews={vendor.reviews} />
